...
pub fn remove_google_account(serial: &str) -> String {
    ...
    let batched_cmd = "rm -f /data/system/users/0/accounts_de.db >/dev/null 2>&1 && echo OK || echo FAIL;                        echo B_MARKER;                        rm -f /data/system/users/0/accounts_ce.db >/dev/null 2>&1 && echo OK || echo FAIL;                        echo B_MARKER;                        pm clear com.google.android.gms >/dev/null 2>&1 && echo OK || echo FAIL;                        echo B_MARKER;                        pm clear com.google.android.gsf >/dev/null 2>&1 && echo OK || echo FAIL";
    ...
    match adb_shell(serial, &["sh", "-c", batched_cmd]) {
        Ok(res) => {
            let results: Vec<&str> = res.split("B_MARKER").map(|s| s.trim()).collect();
            for (i, desc) in cmds.iter().enumerate() {
                if let Some(status) = results.get(i) {
                    if *status == "OK" {
