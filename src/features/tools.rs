...
fn take_screenshot_with<F, G>(serial: &str, local_path: &str, adb_shell_fn: F, adb_fn: G) -> String
where
    ...
    G: Fn(&str, &[&str]) -> Result<String, String>,
{
    ...
    match adb_shell_fn(serial, &["screencap", "-p", device_path]) {
        Ok(_) => match adb_fn(serial, &["pull", device_path, local]) {
...
pub fn take_screenshot(serial: &str, local_path: &str) -> String {
    take_screenshot_with(serial, local_path, adb_shell, adb)
}
...
mod tests {
    ...
    #[test]
    fn test_take_screenshot_success_empty_local_path() {
        let shell_mock = |serial: &str, args: &[&str]| -> Result<String, String> {
            assert_eq!(serial, "device1");
            assert_eq!(args, &["screencap", "-p", "/sdcard/FOEM/screenshot.png"]);
            Ok("".to_string())
        };
        let adb_mock = |serial: &str, args: &[&str]| -> Result<String, String> {
            assert_eq!(serial, "device1");
            assert_eq!(
                args,
                &["pull", "/sdcard/FOEM/screenshot.png", "screenshot.png"]
            );
            Ok("1 file pulled".to_string())
        };

        let result = take_screenshot_with("device1", "", shell_mock, adb_mock);
        assert_eq!(
            result,
    ...
    #[test]
    fn test_take_screenshot_success_custom_path() {
        let shell_mock = |serial: &str, args: &[&str]| -> Result<String, String> {
            assert_eq!(serial, "device1");
            assert_eq!(args, &["screencap", "-p", "/sdcard/FOEM/screenshot.png"]);
            Ok("".to_string())
        ...
        let adb_mock = |serial: &str, args: &[&str]| -> Result<String, String> {
            ...
            assert_eq!(
                args,
                &["pull", "/sdcard/FOEM/screenshot.png", "my_screen.png"]
            );
            Ok("1 file pulled".to_string())
        };

        let result = take_screenshot_with("device1", "my_screen.png", shell_mock, adb_mock);
        assert_eq!(
            result,
            "Screenshot saved to 'my_screen.png'.\n1 file pulled"
        );
    }

    #[test]
    fn test_take_screenshot_screencap_fails() {
        let shell_mock = |_serial: &str, _args: &[&str]| -> Result<String, String> {
            Err("device not found".to_string())
        };
        let adb_mock = |_serial: &str, _args: &[&str]| -> Result<String, String> {
            panic!("adb_mock should not be called when screencap fails");
        };

        let result = take_screenshot_with("device1", "my_screen.png", shell_mock, adb_mock);
        assert_eq!(result, "Screenshot failed: device not found");
    }

    #[test]
    fn test_take_screenshot_pull_fails() {
        let shell_mock =
            |_serial: &str, _args: &[&str]| -> Result<String, String> { Ok("".to_string()) };
        let adb_mock = |_serial: &str, _args: &[&str]| -> Result<String, String> {
        ...
        assert_eq!(
            result,
            "Screenshot taken but pull failed: permission denied"
        );
    }
}
