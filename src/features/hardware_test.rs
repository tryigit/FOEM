...
pub fn test_audio(serial: &str) -> String {
    ...
    if adb_shell(serial, &["media", "volume", "--show"]).is_ok() { output.push_str("  Volume UI triggered.\n") }
    output
}
...
pub fn test_storage(serial: &str) -> String {
    ...
    if let Ok(val) = adb_shell(serial, &["sm", "get-primary-storage-uuid"]) { output.push_str(&format!("  Primary storage UUID: {}\n", val)) }
    output
}
