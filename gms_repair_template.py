...
class DeviceDiagnostics:
    ...
    def detect_device(self):
        ...
        try:
            ...
            for line in lines:
                if line.strip() and 'device' in line and 'offline' not in line:
                    ...
                    self.connected_device = f"ADB_Device_{device_id}"
                    logging.info(f"Detected ADB device: {self.connected_device}")
        ...
        # Try Fastboot second
        try:
            result = subprocess.run(["fastboot", "devices"], capture_output=True, text=True, check=True)
            lines = result.stdout.strip().split('\n')
            for line in lines:
                if line.strip() and 'fastboot' in line:
                    device_id = line.split()[0]
                    self.connected_device = f"Fastboot_Device_{device_id}"
                    logging.info(f"Detected Fastboot device: {self.connected_device}")
                    return self.connected_device
        except Exception:
            pass

        logging.warning("No device detected via ADB or Fastboot.")
        self.connected_device = None
        return self.connected_device
